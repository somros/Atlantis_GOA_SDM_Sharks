...
```{r}
all_boxes <- atlantis_boxes %>% st_set_geometry(NULL) %>% select(.bx0,botz,area,boundary)
...
Write out.
```{r}
write.csv(winter,'salmon_shark_s1.csv', row.names = FALSE)
write.csv(spring,'salmon_shark_s2.csv', row.names = FALSE)
write.csv(summer,'salmon_shark_s3.csv', row.names = FALSE)
write.csv(fall,'salmon_shark_s4.csv', row.names = FALSE)
```
