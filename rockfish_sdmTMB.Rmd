---
title: "Rockfish NPUE with sdmTMB - lat, lon, depth"
...
This document applies the `sdmTMB` routine to the IPHC FISS data filtered to focus on Demersal Shelf and pelagic Shelf rockfish assemblages. This is longline data that cannot be standardized by area like CPUE for the bottom trawl surveys could be. NPUE was calculated as number of individuals per hook for each soak. See file rockfish_IPHC.Rmd for details on how the data for the models was created.

__This is only for comparison purposes. The final spatial distributions for rockfish species are done from bottom trawl survey data.__
...
```{r}
rockfish_data <- readRDS('../data/IPHC/rockfish_rfd.RDS')
```
...
rockfish_bbox <- rockfish_data %>% st_as_sf(coords=c('lon','lat'),crs=4326) %>% st_bbox()

ggplot()+
  geom_point(data = rockfish_data, aes(lon, lat, colour = log1p(npue)), size = 1.5)+
...
```{r}
biom_year <- rockfish_data %>% group_by(year) %>% summarise(npue = sum(npue))
...
rockfish_data <- rockfish_data_sf %>% 
  st_set_geometry(NULL) %>% 
  data.frame(rockfish_coords)
```

Adding an `sf` object of the coastline to incorporate in the mesh.
```{r}
data_bbox <- rockfish_data_sf %>% st_bbox()
...
```{r}
rockfish_spde <- make_mesh(rockfish_data, c("x", "y"), cutoff = 20, type = "cutoff")

# add barrier
rockfish_spde <- add_barrier_mesh(
  spde_obj = rockfish_spde,
...
rockfish_spde$mesh$n
```

Check out the distribution of the biomass response variable.
```{r}
hist(rockfish_data$npue, breaks = 30)
```

```{r}
hist(log1p(rockfish_data$npue), breaks = 30)
```

Proportion of zeroes in percentage.
```{r}
length(which(rockfish_data$npue == 0))/nrow(rockfish_data)*100
```
...
```{r}
rockfish_data$resids <- residuals(m_depth) # randomized quantile residuals
hist(rockfish_data$resids)
```

And QQ plot.
```{r}
qqnorm(rockfish_data$resids)
abline(a = 0, b = 1)
```
...
```{r, fig.width = 12, fig.height=18}
rockfish_sf <- rockfish_data %>% mutate(x = x*1000, y = y*1000) %>% st_as_sf(coords = c(x = "x", y = "y"), crs = atlantis_crs) #%>% st_transform(crs = atlantis_bgm$extra$projection) # turn to spatial object
# 
coast_sf1 <- coast_sf %>% st_transform(crs = atlantis_crs)

ggplot()+
  geom_sf(data = rockfish_sf, aes(color = resids, alpha = .8))+
  scale_color_viridis()+
...
# add year column
all_years <- levels(factor(rockfish_data$year))
...
```{r}
predictions_rockfish <- predict(m_depth, newdata = atlantis_grid, return_tmb_object = TRUE)
atlantis_grid$estimates <- exp(predictions_rockfish$data$est) #Back-transforming here
...
```{r, fig.width = 12, fig.height = 18}
ggplot()+
  geom_sf(data = rockfish_data_sf, aes(colour = log1p(npue)), size = 1.5, alpha = .5)+
...
```{r}
ggplot(data = rockfish_data, aes(x = depth, y = log1p(npue)))+
  scale_color_viridis()+
  geom_point()+
  theme_minimal()+
...
```{r}
ggplot(data = rockfish_data, aes(x = log1p(npue)))+
...
**Demersal shelf assemblage**: This is very similar to what we get out of the bias corrected bottom trawl surveys, except in the IPHC the biomass in BC is higher. 

![RACE sdmTMB](../data/Rockfish_demersal_shelfA.jpg).
...
rockfish_grid_tmp <- rockfish_data %>% dplyr::select(x, y, depth)

# add year
rockfish_grid <- rockfish_grid_tmp[rep(1:nrow(rockfish_grid_tmp), length(all_years)),]
rockfish_grid$year <- as.integer(rep(all_years, each = nrow(rockfish_grid_tmp)))

# predict on this grid
predictions_at_locations <- predict(m_depth, newdata = rockfish_grid, return_tmb_object = TRUE)
rockfish_grid$predictions <- exp(predictions_at_locations$data$est) # back-transforming here
```

Now join by year and coordinates to have predictions at the sampling points. 
```{r, fig.width = 12, fig.height = 6}
rockfish_corr <- rockfish_data %>% left_join(rockfish_grid, by = c("year", "y", "x"))
```

## Observed versus predicted

```{r}
paste0("Pearson's coef observations vs predictions: ", cor(rockfish_corr$npue, rockfish_corr$predictions, use = "everything", method = "pearson"))
```

Plot.
```{r, fig.width = 12, fig.height = 12}
ggplot(rockfish_corr, aes(x = log1p(npue), y = log1p(predictions)))+ # log for visualisation
...
```{r}
paste("RMSE:", sqrt(sum((rockfish_corr$predictions - rockfish_corr$npue)^2)/nrow(rockfish_corr)), " ind hook-1", sep = " ") ### traditional rmse metric, in units kg km2
```

Normalised RMSE. 
```{r}
rmse_cv <- sqrt(sum((rockfish_corr$predictions - rockfish_corr$npue)^2)/nrow(rockfish_corr))/(max(rockfish_corr$npue)-min(rockfish_corr$npue))*100 #### normalised rmse, expressed as a % of the range of observed biomass values, sort of approximates a coefficient of variation 
